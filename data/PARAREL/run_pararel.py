...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str,default="/mnt/e/OneDrive - wqa/Models/llama-2-7b-chat-hf")
    ...
    #
    # with open(f"data/PARAREL/{args.result}_{args.method}_RTuning_train.json",'w') as f:
    #     json.dump(LMFlow_data,f)

    # convert to llama-factory format
    with open(f"data/PARAREL/{args.result}_{args.method}_RTuning_train.json",'r') as f:
        data = json.load(f)['instances']

    for i in range(len(data)):
        text:str=data[i]['text']
        split_pos=text.rfind("Answer:")
        prompt=text[:split_pos]+"Answer: "
        answer_with_postfix=text[split_pos+8:]
        split_pos=answer_with_postfix.find("Are you sure")
        answer = answer_with_postfix[:split_pos]
        split_pos=answer_with_postfix.find("?")
        postfix=answer_with_postfix[split_pos+2:]

        data[i]={'instruction':prompt,'input':"",'output':f"{answer}{postfix}"}
    with open(f"data/{args.result}_{args.method}_RTuning_train.json",'w') as f:

...
"""
It is used to generate the fine-tuning dataset for the Stage1 and Stage2, separately.
"""
...
def generate_cognition_finetuning_dataset(with_qa=False):
    ...
    """
    results = []
    benchmark_dataset, xls_file = load_xls_dataset(os.path.join('data', 'TrustworthyLLM Benchmark.xlsx'))
    benchmark_cleaned: List = benchmark_dataset['benchmark']
    if with_qa:
        qa_dataset = []
        qa_data = json.loads(open(os.path.join("data", 'original', "hotpot_train_v1.1.json")).read())
        qa_i = 0
...
def generate_prompt_sensitive_finetuning_dataset(st=0, end=300):
    ...
    results = []
    dataset, xls_file = load_xls_dataset(os.path.join('data', 'prompt_sensitive_dataset.xlsx'))

    hotpot_dataset = json.loads(
        open(os.path.join('data', 'original', "hotpot_train_v1.1.json")).read())
    ...
    save_xls_dataset(dataset, os.path.join('data', f'prompt_sensitive_dataset.xlsx'),
    ...
    with open(os.path.join('data', f'TrustworthyLLM_PromptSensitive_Finetuning_Dataset.json'), "w") as f:
        json.dump(results_prompt_check, f, indent=2)
    with open(os.path.join('data', f'TrustworthyLLM_QA_Finetuning_Dataset.json'), "w") as f:
        json.dump(results_qa, f, indent=2)
    with open(os.path.join('data', f'TrustworthyLLM_GPT_Finetuning_Dataset.json'), "w") as f:
    ...
    for sample in benchmark_cleaned:
        ...
        results_cong_psqa.append(res)
        psqa_i = (psqa_i + 1) % end
    with open(os.path.join('data', "TrustworthyLLM_Cognition_PSQA_Finetuning_Dataset.json"), 'w') as file:

...
"""
This script is used to generate the trustworthy benchmark dataset for the LLM model.
"""

def generate_trustworthy_benchmark():
    file_path = os.path.join("data", "original", "ASQA.json")
    res = []
    empty_res = []
    clean_dataset = []
    ...
    save_xls_dataset({"benchmark": clean_dataset, 'benchmark_uncleaned': benchmark}, os.path.join('data',"TrustworthyLLM Benchmark.xlsx"),
                     ['id', 'context', 'pos_query', 'pos_answer', 'neg_query'])

def generate_trustworthy_benchmark_coqa():
    file_path = os.path.join("data", "original", "coqa_abg_train.json")
    res = []
    dataset=[]
    with open(file_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    ...
    #save_xls_dataset({"benchmark": res}, os.path.join('data',"TrustworthyLLM Benchmark2.xlsx"),
    #                  ['id', 'context', 'pos_query', 'pos_answer', 'neg_query'])

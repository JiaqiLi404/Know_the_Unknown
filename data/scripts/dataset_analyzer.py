...
"""
Counting the insufficient question-context pairs in separated datasets.
"""

def count_vulnerable_samples(file_name):
    file_path = os.path.join("data", "original", file_name)
    ...
    if file_name == "simplified-nq-train.jsonl":
        ...
        print(
            f"total samples: {len(res)} | vulnerable samples: {len(empty_res)} | proportion:{len(empty_res) / len(res) * 100:.2f}% \n")
    elif file_name == "realtime_qa_data.json":
        file_path = os.path.join("data", file_name)
        data=json.load(open(file_path))
        res=[]
        for d in data:
            if 'don\'t know' in d['answer'].lower():
                res.append(d)
        print(
            f"total samples: {len(data)} | vulnerable samples: {len(res)} | proportion:{len(res) / len(data) * 100:.2f}% \n")
...
count_vulnerable_samples("realtime_qa_data.json")

...
if __name__ == '__main__':
    ...
    for k in dataset.keys():
        ...
        for i in iter_range:
            ...
            answer1,prob1 = call_llm(query=prompt1, model=model, modelName="/mnt/e/OneDrive - wqa/Models/llama-2-7b-chat-hf",trace_logit=True,api=None)
            pos_prob_sum += prob1
            ...
            prompt2 = prompts.get_citation_prompt(neg_query, context, version=prompt_flag,
                                                  task_assignment=task_assignment_prompt)
            answer2,prob2 = call_llm(query=prompt2, model=model, modelName="/mnt/e/OneDrive - wqa/Models/llama-2-7b-chat-hf",trace_logit=True,api=None)
            neg_prob_sum += prob2
        ...
        print("*" * 20, f" positive correct samples: {pos_correct_num} negative correct samples: {neg_correct_num} ",
              "*" * 20)
        print("*" * 15, f" positive reject answering prob: {pos_prob_sum/len(dataset[k]):.3f} negative reject answering prob: {neg_prob_sum/len(dataset[k]):.3f} ",
              "*" * 15)
        print('*' * 80)
        ...
        save_xls_dataset(dataset, os.path.join('benchmark_records', f'TrustworthyLLM Benchmark - {model}.xlsx'),
                         ['id', 'context', 'pos_query', 'pos_answer', 'neg_query', f'{model}_pos_answer',
                          f'{model}_pos_answer_correctness',"pos_prob_of_reject", f'{model}_neg_answer', f'{model}_neg_answer_correctness',"neg_prob_of_reject"])
